...
import { createMaterialFromTextOrUrl, createMaterialFromFile, processExtractedText } from '@/lib/actions';
import { extractTextFromPdf, extractTextFromTxtFile } from '@/lib/pdf-parser';
...
export function CreateMaterialForm() {
  ...
  useEffect(() => {
    const savedActiveTab = localStorage.getItem('cognify_create_active_tab') as ActiveTab | null;
    const savedTitle = localStorage.getItem('cognify_create_title');
    const savedContent = localStorage.getItem('cognify_create_content');
  ...
  useEffect(() => {
    ...
    localStorage.setItem('cognify_create_active_tab', activeTab);
  ...
  useEffect(() => {
    ...
    if (watchedTitle !== undefined) {
      localStorage.setItem('cognify_create_title', watchedTitle);
    }
  }, [watchedTitle, isRestored]);

  // Save content changes
  useEffect(() => {
    if (!isRestored) return;
    if (watchedContent !== undefined) {
      localStorage.setItem('cognify_create_content', watchedContent);
    }
  }, [watchedContent, isRestored]);
  ...
  async function onSubmit(values: any) {
    ...
    startTransition(async () => {
      ...
      try {
            ...
            for (let i = 0; i < files.length; i++) {
                ...
                try {
                    ...
                    if (filenameLower.endsWith('.pdf')) {
                        setCurrentFileStatus(`Extracting text from PDF ${i + 1} of ${files.length}: ${file.name}...`);
                        const rawText = await extractTextFromPdf(file);
                        
                        setCurrentFileStatus(`Processing content with AI ${i + 1} of ${files.length}: ${file.name}...`);
                        const processResult = await processExtractedText(file.name, rawText);
                        if (processResult.error) {
                            throw new Error(processResult.error);
                        }
                        extractedText = processResult.extractedText || '';
                    } else if (filenameLower.endsWith('.txt')) {
                        setCurrentFileStatus(`Reading text file ${i + 1} of ${files.length}: ${file.name}...`);
                        const rawText = await extractTextFromTxtFile(file);
                        ...
                        // For images / docx, send to the server to handle
                        const formData = new FormData();
                        formData.append('file', file);
                        const result = await createMaterialFromFile(formData);
                        if (!result || result.error) {
                            throw new Error(result ? result.error : "Failed to parse file.");
                        }
                        extractedText = result.extractedText || '';
                    }
        ...
        localStorage.removeItem('cognify_create_active_tab');
        localStorage.removeItem('cognify_create_title');
        localStorage.removeItem('cognify_create_content');
                                    ...
                                    <div className="space-y-2">
                                        {Array.from(selectedFiles).map((file, index) => (
                                            <div key={index} className="flex items-center justify-between rounded-md border p-2">
                                                ...
                                                <Button type="button" variant="ghost" size="icon" className="h-6 w-6" onClick={() => form.setValue('files', undefined)} disabled={isUploading}>
                                                    <X className="h-4 w-4" />
                                                </Button>
                                            </div>
                                        ))}
