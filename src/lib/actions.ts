...
export async function processExtractedText(title: string, rawText: string): Promise<ActionResult<any>> {
    try {
        // Check if the content is an outline. If so, expand it.
        const { isOutline } = await isContentOutlineFlow({ content: rawText });
        ...
        console.error(`Failed to process extracted text for "${title}":`, e);
        return { error: `Failed to process text for "${title}": ${e.message || e}` };
    }
}
