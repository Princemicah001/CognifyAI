...
function StudyGuideDisplay({ studyGuide, materialTitle }: { studyGuide: GenerateStudyGuideOutput, materialTitle: string }) {
    ...
    const keyPoints = studyGuide?.keyPoints || [];
    const definitions = studyGuide?.definitions || [];
    const concepts = studyGuide?.concepts || [];
    const examples = studyGuide?.examples || [];
    const mnemonics = studyGuide?.mnemonics || [];
...
function AssessmentDisplay({ assessment, id, onComplete }: { assessment: AIAssessment, id: string, onComplete: (answers: UserAnswer[]) => void }) {
    ...
    useEffect(() => {
        ...
        const savedSessionStr = localStorage.getItem(`cognify_assessment_session_${id}`);
        if (savedSessionStr) {
            try {
                const session = JSON.parse(savedSessionStr);
                const elapsedSeconds = Math.floor((Date.now() - session.timestamp) / 1000);
                let restoredTimeLeft = session.timeLeft;
                let isValid = true;
                if (restoredTimeLeft !== null) {
                    restoredTimeLeft = restoredTimeLeft - elapsedSeconds;
                ...
                if (isValid) {
                    ...
                    localStorage.removeItem(`cognify_assessment_session_${id}`);
    ...
    useEffect(() => {
        ...
        const session = {
            view: { name: 'assessment', data: assessment },
            answers,
            currentQuestionIndex,
            timeLeft,
            timestamp: Date.now()
        };
        localStorage.setItem(`cognify_assessment_session_${id}`, JSON.stringify(session));
    }, [answers, currentQuestionIndex, timeLeft, isRestored, id, assessment]);

    useEffect(() => {
        if (!isRestored || timeLeft === null || timeLeft <= 0) return;
        ...
        const intervalId = setInterval(() => {
            setTimeLeft(prevTime => {
                ...
                if (newTime <= 0) {
                    ...
                    // Clear storage before completing
                    localStorage.removeItem(`cognify_assessment_session_${id}`);
                    onComplete(answers);
                    return 0;
                }
    ...
    }, [timeLeft, onComplete, answers, toast, isRestored, id]);
...
export default function MaterialPage() {
    ...
    // Restore active assessment view on mount if it exists and is still valid
    useEffect(() => {
        if (typeof window === 'undefined') return;
        const savedSessionStr = localStorage.getItem(`cognify_assessment_session_${id}`);
        if (savedSessionStr) {
            try {
                ...
                if (session.timeLeft !== null) {
                    const remaining = session.timeLeft - elapsedSeconds;
                    if (remaining <= 0) {
                        isValid = false;
                        localStorage.removeItem(`cognify_assessment_session_${id}`);
                    }
                }
                if (isValid && session.view?.name === 'assessment') {
                    setView(session.view);
                }
            } catch (e) {
                console.error("Error checking saved assessment session:", e);
            }
        }
    }, [id]);
        ...
        startEvaluationTransition(async () => {
            ...
            try {
                ...
                const overallScore = typeof result?.overallScore === 'number'
                    ? result.overallScore
                    : (typeof (result as any)?.score === 'number' ? (result as any).score : 0);
                ...
                const resultDoc = {
                    ...
                    performanceSummary: `Strengths: ${result?.strengthSummary || 'Completed assessment'}. Weaknesses: ${result?.weaknessAnalysis || 'None identified'}`,
                    recommendations: "Review the feedback provided for incorrect answers.",
                    evaluation: {
                        overallScore: overallScore,
                        strengthSummary: result?.strengthSummary || 'Completed assessment',
                        weaknessAnalysis: result?.weaknessAnalysis || 'None identified',
                ...
                setView({ name: 'results', data: { assessment: assessmentData, evaluation: resultDoc.evaluation as any, userAnswers: answers } });
    ...
    const renderContent = () => {
        switch (view.name) {
            case 'assessment':
                return <AssessmentDisplay assessment={view.data} id={id} onComplete={(answers) => handleAssessmentComplete(answers, view.data)} />;
            case 'results':
